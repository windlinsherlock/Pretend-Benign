...
def inference_single_ckpt(model, data_loader, opencood_dataset, opt, logger, device, ckpt, output_path, hypes):
    ...
    pbar = tqdm(total=len(data_loader), desc='robosac', dynamic_ncols=True)

    sequence_list = hypes['attack_setting']['sequence_list']
    step_budget = hypes['robosac_setting']['step_budget']
    box_matching_thresh = hypes['robosac_setting']['box_matching_thresh']
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            estimated_collab_list, result, step, time_cost = model(batch_data, adv_pert, attacker_list, no_fuse=False, dataset=opencood_dataset, 
                                                        step_budget=step_budget, box_matching_thresh=box_matching_thresh, LCF_mask=LCF_mask, GPS_Attack=GPS_Attack)
    ...
    sample_num = float(len(try_steps))
    avg_step = sum(try_steps) / sample_num
    ...
    logger.info('avg_step: {}'.format(avg_step))
...
def main():
    ...
    log_file = output_path + ('/robosac_%s.log' % (datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))
