...
def inference_single_ckpt(model, data_loader, opencood_dataset, opt, logger, device, ckpt, output_path, hypes):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            heat_maps = model(batch_data, adv_pert, attacker_list, 
                no_fuse=opt.no_fuse, dataset=opencood_dataset, attack=attack, need_box=False, need_heat_map=True, upper_bound=upper_bound)
            
            if opt.save_npy:
                heat_maps_save_path = os.path.join(output_path, 'heat_maps', sample_name)
                if not os.path.exists(heat_maps_save_path):
                    os.makedirs(heat_maps_save_path)
                inference_utils.save_heat_maps(heat_maps, cav_pose, heat_maps_save_path)
        pbar.update(1)
    pbar.close()
...
def main():
    ...
    log_file = output_path + ('/generate_heat_map_%s.log' % (datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))
