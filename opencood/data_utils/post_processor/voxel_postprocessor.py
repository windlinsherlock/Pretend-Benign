...
class VoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train, num_class):
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            prob = output_dict[cav_id]['psm']                                       # (B, anchor_num * num_class , H, W)  
            prob = prob.permute(0, 2, 3, 1)                                         # (B, H, W, anchor_num * num_class) 
            prob = prob.reshape(prob.shape[0], -1, self.num_class)[..., 1:]         # (B, H*W*anchor_num, num_class-1)
            prob = torch.sigmoid(prob)
            prob, _ = torch.max(prob, dim=-1)                                       # (B, H*W*anchor_num)
            prob = prob.reshape(1, -1)                                              # (1, B*H*W*anchor_num)
            
            # regression map
            reg = output_dict[cav_id]['rm']

            # convert regression map back to bounding box
            # (N, W*L*anchor_num, 7)        (x,y,z,h,w,l,r) convert to l, w, h for visualization
            ...
            if len(boxes3d) != 0:
                ...
                pred_box3d_corner_list.append(boxes3d_corner)
                scores_list.append(scores)
                
                boxes3d = boxes3d[:, [0, 1, 2, 5, 4, 3, 6]]             # hwl -> lwh
                pred_box3d_list.append(boxes3d)

        if len(pred_box3d_corner_list) == 0:
            return None, None
        
        # # scores
        scores = torch.cat(scores_list)
        # predicted 3d bbx 
        pred_box3d = torch.vstack(pred_box3d_list)
        pred_box3d_corner = torch.vstack(pred_box3d_corner_list)

        # remove large bbx
        keep_index_1 = box_utils.remove_large_pred_bbx(pred_box3d_corner)
        keep_index_2 = box_utils.remove_bbx_abnormal_z(pred_box3d_corner)
        keep_index = torch.logical_and(keep_index_1, keep_index_2)

        pred_box3d_corner = pred_box3d_corner[keep_index]
        pred_box3d = pred_box3d[keep_index]
        scores = scores[keep_index]

        # nms
        keep_index= iou3d_nms_utils.nms_gpu(pred_box3d, scores, self.params['nms_thresh'], pre_maxsize=1000)

        # select cooresponding box3d
        pred_box3d_corner = pred_box3d_corner[keep_index]
        pred_box3d = pred_box3d[keep_index]
        scores = scores[keep_index]

        # filter out the prediction out of the range.
        mask = box_utils.get_mask_for_boxes_within_range_torch(pred_box3d_corner)

        pred_box3d_corner = pred_box3d_corner[mask]
        pred_box3d = pred_box3d[mask]
        scores = scores[mask]

        assert scores.shape[0] == pred_box3d.shape[0]

        return pred_box3d, scores
