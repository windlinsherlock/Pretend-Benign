...
def inference_single_ckpt(model, data_loader, opencood_dataset, opt, logger, device, ckpt, output_path, hypes):
    ...
    time_cost_list = []
    pbar = tqdm(total=len(data_loader), desc='adv_inference', dynamic_ncols=True)

    sequence_list = hypes['attack_setting']['sequence_list']
    attack = hypes['attack_setting']['attack']
    upper_bound = hypes['attack_setting'].get('upper_bound', False)
    no_fuse = hypes['attack_setting'].get('no_fuse', False)
    GPS_Attack = hypes['attack_setting'].get('GPS_Attack', False)
    logger.info(f'no_fuse: {no_fuse}, attack: {attack}, upper_bound: {upper_bound}, GPS_Attack: {GPS_Attack}')
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if GPS_Attack:
                pred_box_tensor, pred_score, gt_box_tensor, time_cost = model(batch_data, attacker_list, no_fuse=no_fuse, 
                    dataset=opencood_dataset, attack=attack, need_box=True)
            else:
                pred_box_tensor, pred_score, gt_box_tensor, time_cost = model(batch_data, adv_pert, attacker_list, 
                    no_fuse=no_fuse, dataset=opencood_dataset, attack=attack, need_box=True, upper_bound=upper_bound)
...
def main():
    ...
    log_file = output_path + ('/attack_inference_%s.log' % (datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))
