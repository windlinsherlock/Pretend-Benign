...
def inference_single_ckpt(model, data_loader, opencood_dataset, opt, logger, device, ckpt, output_path, hypes):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        if os.path.exists(pert_data_filepath):
            ...
            with torch.no_grad():
                origin_output = model(batch_data, None, None, no_fuse=False, dataset=opencood_dataset, attack=False, need_box=False) 
                psm = origin_output['psm']                                                                      # (1, anchor_num * num_class, H, W)
                ...
                pseudo_neg = fg_prob < max(hypes['postprocess']['target_args']['score_threshold'] - 0.15, 0.05)
                ...
                target_dict['neg_equal_one'] = pseudo_neg.clone().detach()
                target_dict = train_utils.to_device(target_dict, device)
                # print(target_dict['pos_equal_one'].shape, target_dict['pos_equal_one'].sum())
                # print(target_dict['neg_equal_one'].shape, target_dict['neg_equal_one'].sum())
                # exit()
            ...
            for j in range(adv_iter):
                ...
                adv_loss = criterion(origin_output, output_dict, target_dict, anchor_box)
                adv_loss.backward()
...
def main():
    ...
    log_file = output_path + ('/James_attack_%s.log' % (datetime.datetime.now().strftime('%Y%m%d-%H%M%S')))
