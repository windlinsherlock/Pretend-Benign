...
def make_cuda_ext(name, module, sources):
    ...
    pcdet_utils_idx = module_parts.index('pcdet_utils')
    folder_name = os.path.join(*module_parts[pcdet_utils_idx + 1:])
    
    cuda_ext = CUDAExtension(
        name='%s.%s' % (module, name),
        sources=[os.path.join(folder_name, src) for src in sources]
    )
    return cuda_ext
...
setup(
    name='pcd utils',
    package_dir={'opencood.pcdet_utils': '.'},
