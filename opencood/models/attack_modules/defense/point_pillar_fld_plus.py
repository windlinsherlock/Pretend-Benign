...
class FLDPlus(nn.Module):
    def __init__(self):
        super(FLDPlus, self).__init__()
    ...
    def forward(self, spatial_features_2d, dataset, num_cav, batch_data, attacker_list, salient_threshold_I, salient_threshold_U, score_thresh):
        ...
        masked_maps_ego = masked_maps[0].unsqueeze(0).repeat(num_cav, 1, 1)                         # (num_cav, H, W)
        ...
        # selectively accept features that pass FLD defense
        collab_features = spatial_features_2d[estimated_collab]
        collab_heat_maps = heat_maps[estimated_collab]
        ego_heat_map = heat_maps[0]
        num_collab = estimated_collab.shape[0]
        C = collab_features.shape[1]
        
        # collab_features: (n, C, H, W)
        # collab_heat_maps: (n, H, W)
        # ego_heat_map: (H, W)
        
        HCR_mask = ego_heat_map >= 0.7
        AR_mask = (ego_heat_map >= score_thresh) & (ego_heat_map < 0.5)
        IR_mask = ego_heat_map < score_thresh
        
        
        AR_heat = ego_heat_map[AR_mask]
        num_AR = AR_heat.shape[0]
        
        # print(AR_heat.shape, num_AR)
        
        if num_AR > 0:
            AR_heat = sorted(AR_heat)
            mid_index = int(num_AR / 2)
            
            mid_heat = AR_heat[mid_index]
            # print(mid_index, mid_heat)
            
            TAR_mask = (ego_heat_map >= mid_heat) & (ego_heat_map < 0.5)                    # (H, W)
            FAR_mask = (ego_heat_map >= score_thresh) & (ego_heat_map < mid_heat)           # (H, W)
            # print(TAR_mask.sum(), FAR_mask.sum(), HCR_mask.sum(), IR_mask.sum())
            TAR_mask = TAR_mask.unsqueeze(0).repeat(num_collab, 1, 1)                       # (num_collab, H, W)
            FAR_mask = FAR_mask.unsqueeze(0).repeat(num_collab, 1, 1)                       # (num_collab, H, W)
            HCR_mask = HCR_mask.unsqueeze(0).repeat(num_collab, 1, 1)                       # (num_collab, H, W)
            IR_mask = IR_mask.unsqueeze(0).repeat(num_collab, 1, 1)                         # (num_collab, H, W)
            # print(TAR_mask.sum(), FAR_mask.sum(), HCR_mask.sum(), IR_mask.sum())
            
            
            collab_TAR_heats = collab_heat_maps * TAR_mask                                  # (num_collab, H, W)
            collab_FAR_heats = collab_heat_maps * FAR_mask                                  # (num_collab, H, W)
            collab_IR_heats = collab_heat_maps * IR_mask                                    # (num_collab, H, W)
            
            drop_TAR_mask = (collab_TAR_heats < score_thresh) & (collab_TAR_heats > 0)      # (num_collab, H, W)
            drop_FAR_mask = collab_FAR_heats >= score_thresh                                # (num_collab, H, W)
            drop_IR_mask = collab_IR_heats >= score_thresh                                  # (num_collab, H, W)
            # print(drop_TAR_mask.sum(), drop_FAR_mask.sum(), HCR_mask.sum(), drop_IR_mask.sum())
            rand_vals = torch.rand_like(drop_IR_mask.float())                               # (num_collab, H, W)
            drop_IR_mask = drop_IR_mask & (rand_vals < 0.5)
            # print(drop_TAR_mask.sum(), drop_FAR_mask.sum(), HCR_mask.sum(), drop_IR_mask.sum())
            # exit()
            
            drop_TAR_mask = drop_TAR_mask.unsqueeze(1).expand(-1, C, -1, -1)                # (num_collab, C, h, w)
            drop_FAR_mask = drop_FAR_mask.unsqueeze(1).expand(-1, C, -1, -1)                # (num_collab, C, h, w)
            HCR_mask = HCR_mask.unsqueeze(1).expand(-1, C, -1, -1)                          # (num_collab, C, h, w)
            drop_IR_mask = drop_IR_mask.unsqueeze(1).expand(-1, C, -1, -1)                  # (num_collab, C, h, w)
            
            ref = collab_features[0].unsqueeze(0).expand(num_collab, -1, -1, -1)           # (num_collab, C, h, w)
            
            collab_features[drop_TAR_mask] = ref[drop_TAR_mask]
            collab_features[drop_FAR_mask] = ref[drop_FAR_mask]
            collab_features[HCR_mask] = ref[HCR_mask]
            # collab_features[drop_IR_mask] = ref[drop_IR_mask]
...
class PointPillarFLDPlus(nn.Module):
    def __init__(self, args):
        super(PointPillarFLDPlus, self).__init__()
        ...
        self.FLDPlus = FLDPlus()

    
    def forward(self, batch_data, pert, attacker_list, no_fuse, dataset, salient_threshold_I, salient_threshold_U, score_thresh, LCF_mask=None):
        ...
        self.FLDPlus.predict = self.predict
        self.FLDPlus.cls_head = self.cls_head
        self.FLDPlus.fusion_module = self.fusion_module
        
        # start time
        start_time = time.perf_counter()
        
        estimated_collab, succ_result, collab_sim, attacker_sim, collab_match, attacker_match = self.FLDPlus(spatial_features_2d, dataset, num_cav, 
